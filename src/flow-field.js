...
const isInBound = (xCood, yCoord, width, height, margin) => {
  const { minWidth, maxWidth, minHeight, maxHeight } = getBounds(width, height, margin);

  return xCood > minWidth && xCood < maxWidth && yCoord > minHeight && yCoord < maxHeight;
};

export const generateParticles = ({ count, height, margin = 0.1, seed, width }) => {
...
export const moveParticle = ({ amplitude, damping, frequency, lengthOfStep, particle }) => {
...
export const generateField = ({
  ...
  const particles =
    suppliedParticles || generateParticles({ count, height, margin, seed, width }) || [];
